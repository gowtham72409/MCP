...
hitl_manager.py
===============
Lightweight in-process store for Human-in-the-Loop review sessions.

Each pending review is identified by a unique review_id string.
The coroutine that initiated the review suspends on `await_feedback()`
and is resumed when the admin calls `submit_feedback()`.
"""
